window.Modernizr = function(window, document, undefined) {
  ...
  function testProps(props, prefixed) {
    for (var i in props) {
      var prop = props[i];
      if (!contains(prop, "-") && mStyle[prop] !== undefined) return prefixed == "pfx" ? prop : !0;
    }
    return !1;
  }
  ...
  var version = "2.6.1", Modernizr = {}, enableClasses = !0, docElement = document.documentElement, mod = "modernizr", modElem = document.createElement(mod), mStyle = modElem.style, inputElem = document.createElement("input"), smile = ":)", toString = {}.toString, prefixes = " -webkit- -moz- -o- -ms- ".split(" "), omPrefixes = "Webkit Moz O ms", cssomPrefixes = omPrefixes.split(" "), domPrefixes = omPrefixes.toLowerCase().split(" "), ns = {
    ...
    if (parseInt(nodes, 10)) while (nodes--) node = document.createElement("div"), node.id = testnames ? testnames[nodes] : mod + (nodes + 1), div.appendChild(node);
  ...
  !is(_hasOwnProperty, "undefined") && !is(_hasOwnProperty.call, "undefined") ? hasOwnProp = function(object, property) {
    ...
    try {
      if (bool = !!elem.canPlayType) bool = new Boolean(bool), bool.ogg = elem.canPlayType('video/ogg; codecs="theora"').replace(/^no$/, ""), bool.h264 = elem.canPlayType('video/mp4; codecs="avc1.42E01E"').replace(/^no$/, ""), bool.webm = elem.canPlayType('video/webm; codecs="vp8, vorbis"').replace(/^no$/, "");
    } catch (e) {}
    return bool;
  }, tests.audio = function() {
    var elem = document.createElement("audio"), bool = !1;
    try {
      if (bool = !!elem.canPlayType) bool = new Boolean(bool), bool.ogg = elem.canPlayType('audio/ogg; codecs="vorbis"').replace(/^no$/, ""), bool.mp3 = elem.canPlayType("audio/mpeg;").replace(/^no$/, ""), bool.wav = elem.canPlayType('audio/wav; codecs="1"').replace(/^no$/, ""), bool.m4a = (elem.canPlayType("audio/x-m4a;") || elem.canPlayType("audio/aac;")).replace(/^no$/, "");
    } catch (e) {}
    return bool;
  ...
  return Modernizr.input || webforms(), Modernizr.addTest = function(feature, test) {
    if (typeof feature == "object") for (var key in feature) hasOwnProp(feature, key) && Modernizr.addTest(key, feature[key]); else {
      feature = feature.toLowerCase();
      if (Modernizr[feature] !== undefined) return Modernizr;
      test = typeof test == "function" ? test() : test, enableClasses && (docElement.className += " " + (test ? "" : "no-") + feature), Modernizr[feature] = test;
    }
    return Modernizr;
    ...
    function createElement(nodeName, ownerDocument, data) {
      ...
      var node;
      return data.cache[nodeName] ? node = data.cache[nodeName].cloneNode() : saveClones.test(nodeName) ? node = (data.cache[nodeName] = data.createElem(nodeName)).cloneNode() : node = data.createElem(nodeName), node.canHaveChildren && !reSkip.test(nodeName) ? data.frag.appendChild(node) : node;
    }
    ...
    (function() {
      try {
        ...
        a.innerHTML = "<xyz></xyz>", supportsHtml5Styles = "hidden" in a, supportsUnknownElements = a.childNodes.length == 1 || function() {
